...
class Screen:
    ...
    def UpdateButtonLabel(self, index: int, label: str, color: int = None):
        self.labels[index] = label
        self._labelObjs[index][0].text = label
        if color:
            self.labels[index][0].color = color
            self.labels[index][1].color = color
    
    def UpdateButtonColor(self, index: int, color: int):
        # print(self._labelObjs[index][0].color)
        self._labelObjs[index][0].color = color
        self._labelObjs[index][1].color = color
...
def StartBUI():
    ...
    while True:
        ...
        while usb_cdc.data.in_waiting > 0:
            ...
            elif command[0] == 0x03: # set button color  [index] [color]
                data = usb_cdc.data.read(2)
                screen.UpdateButtonColor(data[0], HwScreen.palette[data[1]%16])
                pass
            elif command[0] == 0x04: # write text to screen  [index] [text_color] [bg_color] [x] [y] [xAnchor] [yAnchor] "text"
                if usb_cdc.data.in_waiting > 7:
                    data = usb_cdc.data.read(7)
                    objIdx: int = data[0]
                    textColor: int = data[1]
                    boxColor: int = data[2]
                    # xPos: int = data[2]
                    # yPos: int = data[3]
                    position = (data[3], data[4])
                    anchor = (data[5]/255, data[6]/255)
                    # xAnchor = data[4]/256
                    # yAnchor = data[5]/256
                    data = usb_cdc.data.readline()
                    text = data.decode().rstrip()
                    color = HwScreen.palette[textColor]
                    backgroundColor = None if boxColor == 0x00 else HwScreen.palette[boxColor%16]
            ...
            elif command[0] == 0x05: # write scrolling text to screen   [index] [text_color] [bg_color] [x] [y] [xAnchor] [yAnchor] [scrollSpeedSeconds] [scrollSpeedSecondFraction] [max_chars] "text"
                if usb_cdc.data.in_waiting > 10:
                    data = usb_cdc.data.read(10)
                    objIdx: int = data[0]
                    textColor: int = data[1]
                    boxColor: int = data[2]
                    # xPos: int = data[2]
                    ...
                    position = (data[3], data[4])
                    anchor = (data[5]/255, data[6]/255)
                    seconds = float(data[7])+(1/data[8] if data[8] != 0 else 0)
                    maxchars = data[9]
                    # xAnchor = data[4]/256
                    # yAnchor = data[5]/256
                    data = usb_cdc.data.readline()
